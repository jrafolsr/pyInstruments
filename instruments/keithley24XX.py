...
#        self.inst.write(":SOUR:VOLT 0.00")       # Set source to output 0.0V.
    ...
    def configure_syncsweep_master(self,start, stop, step = 0.1, mode = 'step', sweep_list = [], term = 'FRONT', cmpl = 0.1, delay = 0.1, ranging = 'AUTO', nplc = 1, spacing = 'LIN', reset = True, stay_on = False, source_range = 'BEST'):
        """
        Configures the Keithley to perform a voltage sweep
...
#        self.inst.write("sour:cle:auto:mode tco") # Output off after trigger count
        
        self.inst.write("arm:coun 1") # Perform 1 sweep per ":INIT"
        self.inst.write("arm:sour imm") # Immediately go to Trig layer
        self.inst.write("arm:dir ACC") # Wait for trigger event (IMM)
        self.inst.write("arm:outp none") # No output triggers from scan
#        self.inst.write("trig:coun 10") # Perform # points in sweep
        ...
        self.inst.write("trig:del 0.0") # Set trigger delay before source
...
#        self.inst.write("sour:cle:auto on") # Automatically turn output ON/OFF
        ...
        self.inst.write("arm:coun 1") # Perform only one scan
        self.inst.write("arm:sour imm") # Immediately start scan
        self.inst.write("arm:dir acc") # Wait for trigger event (IMM)
        self.inst.write("arm:outp none") # No output triggers from scan
        self.inst.write("trig:coun {:d}".format(Npoints)) # Perform # points in sweep
        self.inst.write("trig:sour tlin") # Trigger using Trigger Link
        self.inst.write("trig:dir acc") # Wait for first trigger
        self.inst.write("trig:inp sour") # Wait for trig before source "on"
        self.inst.write("trig:outp del") # Output trigger after delay
        self.inst.write("trig:ilin 2") # Input trigger line
        self.inst.write("trig:olin 1") # Output trigger line
        self.inst.write("trig:del {:.4f}".format(trig_delay)) # Set trigger delay before source
        
        self.inst.write("sour:func volt") # Output constant voltage
        self.inst.write("sens:func 'curr'") # Measure leakage current of PD
        self.inst.write("sour:curr:rang:auto on") # Enable source auto range
        self.inst.write("sour:volt {:.4f}".format(bias_voltage)) # Set output level
        self.inst.write("sour:del {:.4f}".format(source_delay)) # Set source delay before measure
        self.inst.write("sens:curr:prot {:.4e}".format(curr_cmpl)) # 10mA compliance range
#        self.inst.write("sens:curr:rang:AUTO ON") # Measure on the specified range
        ...
        self.inst.write("sens:curr:nplc {:.4f}".format(nplc)) # Measurement integration
        
#        self.inst.write('INIT')
        
        
    def fetch(self, delay = None):
        
        if delay == None:
            delay = self.sweep_total_time
        # Fetch the data
        return self.inst.query_ascii_values('FETCH?',delay = delay, container = array)  # Assumes data is in ASCII format
