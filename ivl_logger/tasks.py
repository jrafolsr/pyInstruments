...
from pyInstruments.ivl_logger import PH_ADDRESS, PH_GAIN, PH_CHANNELS_CONFIG
# Photodiode module
from pyInstruments.ads11X5_logger import ADS11x5Logger
# Thread module to launch the photodiode
...
class IVL_LoggerTask(object):
    def __init__(self, resource = None, folder = Path.home(), filename = 'IVLlogger-data',\
        ...
        self.ph_voltage = deque([], self.max_length)
    ...
    def start_photodiode(self, photodiode_gain = PH_GAIN):
        # Starting photdiodes
        self.ph = ADS11x5Logger(channels_config = PH_CHANNELS_CONFIG, gain = photodiode_gain, address = PH_ADDRESS)
        sleep(0.25)
        # Start photodiode readings
        ph_thread = Thread(target = self.ph.measure, kwargs = dict(continuous = True, show_print = True))
    ...
    def run_photodiode_only(self,  runtime = np.inf, dt = 0.25, dt_fix = True):
        """
        Only runs the photodiode, no sourcemeter resource expected
        
        runtime : float, optional
            Running time of the experiment in seconds. The default is np.inf.
        dt : float, optional
            Time inverval between measurements. The default is 0.25.   
        ...
        if not filename.exists():
            with open(filename,'a') as f:
                f.write(('# ' + 2*'{:^12}\t' + '\n').format('Time', 'Photodiode_voltage'))
                f.write(('# ' + 2*'{:^12}\t' + '\n').format('s','V'))
        ...
        while etime < runtime and self.running:
            try:
                ...
                self.voltage.append(np.nan)
                self.ph_voltage.append(m_phvoltage)
    ...
    def run(self, value, runtime = np.inf, dt = 0.25, interrupt_measurement = True, dt_fix = True):
        ...
        if not filename.exists():
            with open(filename,'a') as f:
                f.write(('# {:^5}\t'+5*'{:^12}\t' + '\n').format('Step','Time','Current','Voltage', 'Absolute time', 'Photodiode_voltage'))
                f.write(('# {:^5}\t'+5*'{:^12}\t' + '\n').format('i','s','A', 'V', 's','V'))
    ...
    def stop_photodiode(self):
        self.ph.measurement_off()