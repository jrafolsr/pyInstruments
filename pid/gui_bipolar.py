...
Bipolar update:
    - Removed the cooling/heating BooleanSwitch. Direction is now decided
      per-step by the controller from the sign of (setpoint - T).
    - Max action voltage capped at 6 V (the P6V channel's ceiling, which
      limits the bridge swing).
    - start_instrument callback no longer passes a heating flag.
    - Added a current-compliance NumericInput so the per-channel current limit
      can be set from the GUI.
"""
...
p = TemperatureControllerBipolar()
...
app.layout = html.Div(children = [
    html.Div(className = 'row', children = [
        html.Div(className = 'column left', children = [
            ...
            html.H4('Temperature stage PID setup (bipolar)',
        ...
        html.Div(className = 'column middle', children = [
            ...
            html.P(['Sensor type:',
                    ...
                    # NOTE: cooling-switch removed. Direction is now decided
                    # per-step from the sign of (setpoint - T) inside the
                    # TemperatureController.
                    daq.NumericInput(
                        id='max-power',
                        label = 'Max. action |V|',
                        min = 0.0,
                        max = 2.00,        # bridge is capped by the 6 V channel
                        value = 2.00,
                    ),
                    daq.NumericInput(
                        id='current-limit',
                        ...
                        min = 0.0,
                        max = 1.5,         # Keithley maxes at 3 A, but not sure about te Peltier
                        value = 1.0,
                    )
                    ]),
        ]),
        ...
        html.Div(className = 'column right', children = [
            html.Div([
                html.P(id = 'label-setpoint',
                       children = 'Temperature setpoint is 20.00 °C'),
                dcc.Input(
                    id='set-setpoint',
                    type = 'number',
                    min = -20,             # bipolar drive can now cool below ambient
                    max = 100,
                    value = 20.00,
                    debounce = True,
                    size = '10'
...
@app.callback([Output('my-daq-startbutton', 'disabled'),
              ...
              [State('my-daq-startbutton', 'n_clicks'),
               ...
               State('current-limit', 'value'),
               State('dropdown-multimeter', 'value'),
               State('dropdown-sourcemeter', 'value'),
               State('rtd-type', 'value')])
def start_instrument(on, N, setpoint, max_power, current_limit,
                     mult_addr, source_addr, R0):
    """Configure and arm the temperature controller.

    Bipolar drive: no heating/cooling flag is needed -- the controller
    decides direction per-step from the sign of (setpoint - T).
    """
    if on is None:
        return True, 0
    try:
        if on:
            p.configurate(setpoint = setpoint,
                          max_poutput = max_power,
                          current_compliance = current_limit,
                          multimeter_addr = mult_addr,
                          sourcemeter_addr = source_addr,
                          R0 = R0)
            return False, N
        else:
            return True, 0
...
@app.callback([Output('current-limit', 'label')],
              [Input('current-limit', 'value')],
              prevent_initial_call = True)
def max_current(value):
    p.set_current_limit(value)
    label = 'Current limit (A)'
    return [label]
